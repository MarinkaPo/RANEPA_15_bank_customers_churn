...
\n Задания *первой части лабораторной работы* будут включать 3 блока: анализ данных, их предобработку и визуализацию.
...
\n 1. Сколько всего клиентов пользуются услугами вашего отделения банка? 
\n 2. Какое количество данных (количество признаков) собирает банк по каждому клиенту?
\n 3. Какой средний возраст ваших клиентов?
\n 4. С какого возраста клиента банк заключает с ним договор на обслуживание?
\n 5. Как варьируется баланс на счетах клиентов банка (от минимального до максимального)?
\n 6. Доход ниже какой суммы имеет половина клиентов банка?
\n 7. Какая доля клиентов обслуживается в банке 2 года и менее?
\n 8. Каким максимальным числом продуктов банка пользуются клиенты?
\n 9. Верно ли, что средний показатель баланса на счёте имеет половина клиентов банка? 
\n 10. Как вы считаете, почему такие характеристика, как наличие кредитной карты, город проживания, пол и активность клиента называются "категориальными перменными"? 
В чём, по вашему мнению, их отличие от таких характеристик, как баланс на счёте, возраст и предполагаемая зарплата клиента?''')
...
st.markdown('''<h2 style="text-align: left; color: black;">Блок 2: Предобработка данных </h2>''', 
            unsafe_allow_html=True)
st.markdown(''' \nКак вы думаете:
\n - все ли собираемые по клиентам данные полезны для прогноза их поведения?
\n - всегда ли данные собираются в удобном для математичекой обрабоки виде?''')
if st.button('Подсказка и ответ', key='hint1'):
  st.write('Ответ на оба вопроса - "**Нет, не всегда**". Часто данные собираются в понятном для человека, но не подходящем для работы математической модели виде. А часть имеющихся данных вообще может не влиять на прогноз.')

st.markdown('''\n *Задание по Блоку 2:*
\n1. Попробуйте выбрать признаки, которые по вашему мнению НЕ повлияют на прогноз поведения клиента''')
genre = st.radio("Какие собранные характеристики клиентов не принципиальны для прогноза их поведения?", 
...
st.markdown('''\n2. Предобработайте данные для их последующего анализа
\nДля чего это нужно?
\nТак как в математическую модель для прогноза могут подаваться данные определённого, числового вида, 
изначальная (описанная обычным языком) форма данных не подойдет: категориальные переменные (пол, наличие кредитной карты, город проживания, активность, - т.е.
 те, которые связаны с определённым качеством или категорией клиента) нужно обработать.
 \nТакже, нужно удалить ненужные данные, которые вы выбрали в предыдущем пункте, т.к. они не повлияют на прогноз поведения клиента.''')
preprocessing_click =  st.checkbox('Удалить ненужные колонки и обработать категориальные переменные')
if preprocessing_click:
  df_ohe = df.drop(['CustomerId','Surname'], axis=1, inplace=True)
  # заэнкодим:
  ...
  st.markdown(f'''*отображены первые 5 строк таблиц:* 
  \n**Было:** весь размер таблицы: {df.shape[0]} строк, {df.shape[1]} колонок''')
  st.write(df.head(5), use_column_width='auto')
  st.markdown(f'''**Стало:** весь размер таблицы: {df_ohe.shape[0]} строк, {df_ohe.shape[1]} колонок''')
  st.write(df_ohe.head(5), use_column_width='auto')

st.markdown('''<h2 style="text-align: left; color: black;">Блок 3: Графики и зависимости в данных: </h2>''', 
            unsafe_allow_html=True)
...
st.markdown('''<h2 style="text-align: left; color: black;"> Выводы по первой части лабораторной работы: </h2>''', 
            unsafe_allow_html=True)
